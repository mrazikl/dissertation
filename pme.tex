...
\end{equation}
zavádíme hustotu \Unit{\rho}{\kilogram\per\cubic\meter} a~vektor rychlosti \Unit{\bm{v}}{\meter\per\second}. 
...
\end{equation}
zavádíme měrnou vnitřní energii \Unit{u}{\joule\per\kilogram}, Cauchyho tenzor napětí \Unit{\bm{\sigma}}{\pascal}, měrnou entalpii \Unit{h}{\joule\per\kilogram}, vektor hustoty tepelného toku \Unit{\bm{q}}{\watt\per\square\meter} a~vektor hustoty toku chemické energie \Unit{\bm{\phi}}{\watt\per\square\meter}. 
...
\end{equation}
kde $\bm{I}$ je jednotkový tenzor, zavádíme tenzor viskózního (též smykového) napětí \Unit{\bm{\tau}}{\pascal} a~tlak \Unit{p}{\pascal}.

\paragraph{Dynamická viskozita} je materiálový parametr vyjadřující odpor plynu vůči smykové deformaci, zavádíme \Unit{\mu}{\pascal\second}. Má svůj původ v~kinetické teorii plynů, je důsledkem přenosu hybnosti mezi molekulami. Pro účely našeho modelu budeme dynamickou viskozitu považovat za~konstantní. 
...
\end{equation}
kde \Unit{\zeta}{\pascal\second} je objemová viskozita, kterou budeme uvažovat konstantní.
...
\begin{equation}
    \bm{q}=-\lambda \nabla T,
\end{equation}
zavádíme tepelnou vodivost \Unit{\lambda}{\watt\per\meter\per\kelvin} a~teplotu \Unit{T}{\kelvin}. 
...
\begin{equation}\label{eq:ideal_gas_law}
    p V = m R_s T, \qquad p=\rho R_s T,
\end{equation}
zavádíme hmotnost \Unit{m}{\kilogram} a~měrnou plynovou konstantu \Unit{R_s}{\joule\per\kilogram\per\kelvin}.
...
\begin{equation}\label{eq:gibbs_equation}
    \d{u}=T\d{s}-p\d{\nu},
\end{equation}
kde zavádíme entropii \Unit{s}{\joule\per\kilogram\per\kelvin} a měrný objem \Unit{\nu}{\cubic\meter\per\kilogram}.

\paragraph{Mayerův vztah} svazuje měrné tepelné kapacity ideálního plynu,
\begin{equation}\label{eq:mayer}
    c_p - c_V = R_s, \qquad c_p=\left(\dd{h}{T}\right)_p, \qquad c_V=\left(\dd{u}{T}\right)_V,
\end{equation}
zavádíme měrnou tepelnou kapacitu při konstantním tlaku \Unit{c_p}{\joule\per\kilogram\per\kelvin} a~při konstantním objemu \Unit{c_V}{\joule\per\kilogram\per\kelvin}. Budeme předpokládat, že~plyn je kaloricky dokonalý, tj. jeho tepelné kapacity jsou konstantní.
...
Definujme nyní tenzor permeability \Unit{k}{\square\meter} po~složkách jako
\begin{equation}
...
\begin{equation}
    \rho h v_x=\rho c_p T v_x = \rho c_p \frac{p}{R_s \rho}v_x=\frac{c_p}{R_s}p v_x=-\frac{c_p}{R_s}p\frac{k}{\mu}\dd{p}{x}.
\end{equation}
Získáváme vyjádření konvektivního členu,
\begin{equation}\label{eq:convection_io_mean}
    -\int_{\partial\mathcal{I}+\partial\mathcal{O}} \mkern-36mu \rho h \bm{v} \cdot \dvec{S}
    ={\left[\varepsilon A\frac{k}{\mu}\frac{c_p}{R_s}\left\langle p\dd{p}{x}\right\rangle\right]}_0^L.
\end{equation}
...
\begin{equation}
    \rho u~= \rho c_V T = \rho c_V \frac{p}{R_s \rho}=\frac{c_V}{R_s}p.
\end{equation}
Dosazením získáváme vyjádření akumulačního členu,
\begin{equation}\label{eq:accumulation_mean}
    \dd{}{t}\int_{\mathcal{G}}\rho u~\d{V}=\int_0^L\varepsilon A\frac{c_V}{R_s}\dd{\left\langle p\right\rangle}{t}\d{x}.
\end{equation}
...
Zavedeme substituci \Unit{J}{\pascal\per\second} či ekvivalentně \Unit{\!}{\watt\per\cubic\meter},
\begin{equation}
...
\begin{equation}
    \rho(x)=\frac{p(x)}{R_s T}=\frac{p_{\mathrm{in}}}{R_s T}S_2^{\frac{1}{2}}(x).
\end{equation}
Z rovnice kontinuity dále odečteme hustotu toku plynu membránou \Unit{j}{\kilogram\per\square\meter\per\second}, neboť po~dosazení Darcyho zákona platí
\begin{equation}
    j=\rho v~= -\frac{k}{\mu}\rho \Dd{p}{x}
    =-\frac{1}{2}\frac{k}{\mu}\frac{p_{\mathrm{in}}^2}{R_s T}S'_2S_2^{\frac{1}{2}-\frac{1}{2}}(x)
    =\frac{k\left(p_{\mathrm{in}}^2-p_{\mathrm{out}}^2\right)}{2\mu R_s TL}.
\end{equation}
...
\begin{equation}
    j=\rho v = \frac{p}{R_s T}\cdot-\frac{k}{\mu}\Dd{p}{x}=-\frac{k}{\mu R_s}\frac{p\Dd{p}{x}}{T}\quad\Longrightarrow \quad
...
\begin{equation}
    \rho(x)=\frac{p(x)}{R_sT(x)}
    =\frac{p_{\mathrm{in}}S_\alpha^{\frac{1}{\alpha}}(x)}{R_s T_{\mathrm{in}}S_\alpha^{\frac{2}{\alpha}-1}(x)}
    =\frac{p_{\mathrm{in}}}{R_sT_{\mathrm{in}}}S_\alpha^{1-\frac{1}{\alpha}}(x).
\end{equation}
...
\begin{equation}
    ...
    \end{split}
\end{equation}
Pozorujeme, že~indukovaná hustota výkonu je lineární funkcí hustoty mechanického výkonu plynu (viskózní disipace) \Unit{\Phi}{\watt\per\cubic\meter},
\begin{equation}
...
\begin{equation}\label{eq:pme_mass_flux}
    j=\frac{k p_{\mathrm{in}}^{2-\alpha}}{\alpha \mu R_s L T_{\mathrm{in}}}\left(p_{\mathrm{in}}^{\alpha}-p_{\mathrm{out}}^{\alpha}\right)
    =\frac{k p_{\mathrm{in}}^{2}}{\alpha \mu R_s L T_{\mathrm{in}}}\left(1-\left(\frac{p_{\mathrm{out}}}{p_{\mathrm{in}}}\right)^{\alpha}\right).
\end{equation}
...
\begin{equation}
    \nu=\frac{1}{\rho}=\frac{R_s T}{p},\quad \d{\nu}=\frac{R_s}{p}\d{T}-\frac{R_s T}{p^2}\d{p}=\frac{R_s }{p}\d{T}-\frac{\nu}{p}\d{p},
\end{equation}
...
\begin{equation}
    T\d{s}=\d{u}+p\d{\nu}=c_V\d{T}+R_s\d{T}-\nu\d{p}=c_p\d{T}-\frac{R_s T}{p}\d{p}.
\end{equation}
...
\begin{equation}
    \Dd{s}{\xi}=\dd{s}{T}\Dd{T}{\xi}+\dd{s}{p}\Dd{p}{\xi}=\frac{c_p}{T}\Dd{T}{\xi}-\frac{R_s}{p}\Dd{p}{\xi}.
\end{equation}
...
\begin{equation}
    T\left.\Dd{s}{T}\right|_{\mathcal{C}}=c_p - \frac{R_s T}{p}\left.\Dd{p}{T}\right|_{\mathcal{C}}=c_p - \nu\left.\Dd{p}{T}\right|_{\mathcal{C}},
\end{equation}
zavádíme odchylku měrné tepelné kapacity od izobarického děje \Unit{c_d}{\joule\per\kilogram\per\kelvin},
\begin{equation}
...
\begin{equation}
    \left(c_p-c_d\right)\d{T}=c_p\d{T}-\frac{RT}{p}\d{p},\qquad \frac{\d{T}}{T}=\frac{R_s}{c_d}\frac{\d{p}}{p},
\end{equation}
kde poměr $R_s/c_d$ budeme dále používat jako bezrozměrný parametr vhodný k~popisu odchylky od izobarického děje,
\begin{equation}
    \chi=\frac{R_s}{c_d}=R\rho \frac{\d{T}/\!\d{\xi}}{\d{p}/\!\d{\xi}}.
\end{equation}

Abychom získali diferenciální formu svazující tlak a~hustotu, vyjádříme úplný diferenciál stavové rovnice ideálního plynu,
\begin{equation}
    \d{p} = R_s\left(T\d{\rho} + \rho\d{T}\right) \quad \Longrightarrow \quad \frac{\d{p}}{p}=\frac{\d{\rho}}{\rho}+\frac{\d{T}}{T}.
\end{equation}
...
\begin{equation}
    n=\frac{1}{1-\chi}=\frac{c_d}{c_d-R_s}.
\end{equation}
...
\begin{subequations}
    \begin{align}
        &&&&p_0&=\rho_0 R_s T_0 = K_\rho \rho_0^n &\Longrightarrow\qquad  K_\rho  &= \rho_0^{1-n} R_s T_0,&&\\
        &&&&\rho_0&=\frac{p_0}{R_s T_0}=K_p p_0^{\frac{1}{n}}&\Longrightarrow\qquad  K_p &= \frac{1}{R_s T_0}p_0^{\frac{n-1}{n}}.&&
    \end{align}
\end{subequations}
...
\begin{subequations}
    ...
    ,$&&\refstepcounter{equation}(\theequation)\\~\\ &
          $\dst \frac{1}{\alpha-1}
    $&$=$&$\dst n
    $&$=$&$\dst \frac{c_d}{c_d-R_s}
    $&$=$&$\dst \frac{1}{1-\chi}
    ,$&&\refstepcounter{equation}(\theequation)\\~\\ &
          $\dst \frac{R_s}{2-\alpha}
    $&$=$&$\dst \frac{n R_s}{n-1}
    $&$=$&$\dst c_d
    $&$=$&$\dst \frac{R_s}{\chi}
    ,$&&\refstepcounter{equation}(\theequation)\\~\\ &
...
\begin{subequations}
    \begin{align}
        p&=K_{\rho} \rho^{\frac{1}{\alpha-1}},\qquad K_\rho = R_s T_0\rho_0^{\frac{\alpha-2}{\alpha-1}},\\
        \rho&=K_p p^{\alpha-1},\qquad K_p=\frac{1}{R_s T_0} p_0^{2-\alpha},
    \end{align}
\end{subequations}
profil hustoty pak získáváme v~podobě
\begin{equation}
    \rho(x)=\frac{p_0^{2-\alpha}}{R_s T_0}p_{\mathrm{in}}^{\alpha-1}\left(1-\frac{x}{L}\left(1-\left(\frac{p_{\mathrm{out}}}{p_{\mathrm{in}}}\right)^{\alpha}\right)\right)^{\frac{\alpha-1}{\alpha}}.
\end{equation}
...
\begin{equation}\label{eq:pme_mass_flux_reference}
    ...
    =\frac{1}{L}\frac{k}{\mu}\frac{p_0^{2-\alpha}}{R_s T_0}\frac{p_{\mathrm{in}}^{\alpha}}{\alpha}\left(1-\left(\frac{p_{\mathrm{out}}}{p_{\mathrm{in}}}\right)^{\alpha}\right).
\end{equation}
